...
def scrape_fight_data(engine):
    try:
        ...
        missing_fights = fight_df[
            ~fight_df["fight_url"].isin(fight_data_df["fight_url"])
        ]
        ...
        for fight_url in missing_fights["fight_url"]:
            ...
            if len(fighters) != 2:
                logging.warning(f"Invalid fighter count ({len(fighters)}): {fight_url}")
                continue

            f1 = fighters[0].select_one("h3").get_text(strip=True)
            f2 = fighters[1].select_one("h3").get_text(strip=True)
            ...
            num_rounds = 0
            time_format = next(
                (p for p in soup.select("p.b-fight-details__text") if "Time format:" in p.text),
                None
            )
            if time_format:
                match = re.search(r"(\d+)\s*Rnd", time_format.text)
                if match:
                    num_rounds = int(match.group(1))

            weight_tag = soup.select_one("i.b-fight-details__fight-title")
            weight_class = weight_tag.get_text(strip=True).replace(" Bout", "") if weight_tag else None
            title_fight = bool(weight_class and "Title" in weight_class)
...
def insert_fight_data(fight_data, engine):
    ...
    try:
        ...
        with engine.begin() as conn:
            for chunk in chunk_dataframe(fight_data, size=100):
                stmt = (
                    insert(fight_data_table)
                    .values(chunk.to_dict(orient="records"))
                    .on_conflict_do_nothing(index_elements=["fight_url"])
                )

                result = conn.execute(stmt)
                total_inserted += result.rowcount

        logging.info(f"Inserted {total_inserted} new fight_data rows.")

    except Exception as e:
        logging.error(f"Error inserting fight data: {e}")
        raise
