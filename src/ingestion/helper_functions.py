...
def get_latest_event():
    ...
    try:
        ...
        event_link = soup.select_one(
            'i.b-statistics__table-content a.b-link[href*="event-details"]'
        )
...
def get_event_details(event_url):
    try:
        ...
        if not event_name_tag:
            logging.warning(f"Missing event name for {event_url}")
            return []
        ...
        for row in fight_rows:
            ...
            fight_data = {
                ...
                "weight_class": (
                    row.select_one("td:nth-of-type(7) p").get_text(strip=True)
                    if row.select_one("td:nth-of-type(7) p")
                    else "N/A"
                ),
            }
...
def parse_height(h):
    return None if "--" in h else int(
        (int(h[0]) * 12 + int(h.split("'")[1].strip().strip('"'))) * 2.54
    )
...
def parse_dob(d):
    return None if d == "--" else datetime.strptime(
        d,
        "%b %d, %Y"
    ).strftime("%Y-%m-%d")
...
def scrape_one_fight(url):
    try:
        soup = get_soup_with_playwright(
            url,
            parser="html.parser",
            selector="p.b-fight-details__table-text"
        )
...
def scrape_fighter_from_url(url):
    try:
        soup = get_soup_with_playwright(
            url,
            parser="lxml",
            selector="li.b-list__box-list-item"
        )
